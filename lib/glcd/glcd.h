/*********************************************************************************************
 * Osama's Lab KS0108 GLCD library; a library that facilitates interfacing
 * KS0108 based GLCDs.
 *
 *   Copyright (C) 2010-2012  Osama's Lab
 *   Copyright (C) 2026  Yaroslav Lobachev, MIREA Lab
 *
 *   Modified 2026 by Yaroslav Lobachev: LLVM-style formatting, added
 *   `draw_main` and `draw_admin` functions.
 *
...
//-----------------------
void rectangle(unsigned int x1, unsigned int y1, unsigned int x2,
               unsigned int y2, byte s, byte c) {
...
//-----------------------
void cuboid(unsigned int x11, unsigned int y11, unsigned int x12,
            unsigned int y12, unsigned int x21, unsigned int y21,
            unsigned int x22, unsigned int y22, byte s, byte c) {
...
//-----------------------
void h_parallelogram(unsigned int x1, unsigned int y1, unsigned int x2,
                     unsigned int y2, byte l, byte s, byte c) {
    h_line(x1, y1, l, s, c);
...
//-----------------------
void v_parallelogram(unsigned int x1, unsigned int y1, unsigned int x2,
                     unsigned int y2, byte l, byte s, byte c) {
    v_line(x1, y1, l, s, c);
...
//-----------------------
void h_parallelepiped(unsigned int x11, unsigned int y11, unsigned int x12,
                      unsigned int y12, byte l1, unsigned int x21,
                      unsigned int y21, unsigned int x22, unsigned int y22,
                      byte l2, byte s, byte c) {
...
//-----------------------
void v_parallelepiped(unsigned int x11, unsigned int y11, unsigned int x12,
                      unsigned int y12, byte l1, unsigned int x21,
                      unsigned int y21, unsigned int x22, unsigned int y22,
                      byte l2, byte s, byte c) {
...
void putItSz(int c, int x, int y, byte sz) {
    ...
    for (j = 0; j < sz; j++) {
        for (i = 0; i < 8; i++) {
            for (k = 0; k < sz; k++) {
                if (x + k + (sz * i) == 64)
                    goto_xy(64, y); // Problem 1: Forgot to change 3 to sz
...
void glcd_putchar(byte c, int x, int y, byte l, byte sz) {
    if (l == 1) {
        ...
        if ((c >= 193) && (prevLet >= 193) &&
            (pgm_read_byte(&(map[prevLet - 193][5]))) &&
            (pgm_read_byte(&(map[c - 193][4])))) {
            putItSz(pgm_read_byte(&(map[prevLet - 193][stat + 1])), prevX,
                    prevY, sz);
            stat = 2;
        } else
            stat = 0;
        ...
        if (c >= 193)
            putItSz(pgm_read_byte(&(map[c - 193][stat])), x, y, sz);
        else
            putItSz(c, x, y, sz);
...
//--------------------------
void glcd_puts(char *c, int x, int y, unsigned char l, byte sz,
               signed char space) {
    char i = 0;
    char special = 0;
...
//------------BMP Display--------------------
void bmp_disp(byte *bmp, unsigned int x1, unsigned int y1, unsigned int x2,
              unsigned int y2) {
    unsigned int i, j;
    for (i = y1; i <= y2; i++) {
        for (j = x1; j <= x2; j++) {
...
//-------------Draw Main Screen-------------
void draw_main(uint8_t user_found, char user_name[]) {
    glcd_clear();
